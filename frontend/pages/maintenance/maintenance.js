...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("modal-form").addEventListener("submit", async function(event) {
    ...
    let payload = {};
    payload = { vehicleId: parseInt(document.getElementById("form-vehicle").value), description: document.getElementById("form-description").value.trim(), cost: parseFloat(document.getElementById("form-cost").value), date: document.getElementById("form-date").value, status: document.getElementById("form-status").value };
    try {
      let url = `/api/${type}`;
      let method = "POST";
