...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("modal-form").addEventListener("submit", async function(event) {
    ...
    let payload = {};
    payload = { name: document.getElementById("form-name").value.trim(), license: document.getElementById("form-license").value.trim(), phone: document.getElementById("form-phone").value.trim(), status: document.getElementById("form-status").value };
    try {
      let url = `/api/${type}`;
      let method = "POST";
